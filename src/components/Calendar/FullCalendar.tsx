...
import { useStudyGoalAllGet, useStudyGoalDelete } from '../../hooks/react_query_hooks/useStudyGoal';
...
const FullCalendar = () => {
  ...
  const { deleteGoal }= useStudyGoalDelete();

  useEffect(() => {
      setEventList(data?.data.data.map((e: any) => ({ ...e, start: e.startDate, end: e.endDate }))||[]);
  }, [data?.data.data]);
